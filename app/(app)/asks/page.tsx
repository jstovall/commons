...
export default async function AsksPage({
  ...
  const { current: membership } = await getCurrentMembership(userId);
  ...
  const selectFields = `id, title, description, status, created_at, requester_id, content_flag,
  ...
  if (askParam) {
    ...
    if (!isAdmin) {
      query = query.eq("content_flag", false);
    }

    const result = await query.maybeSingle();
    asks = result.data ? [result.data] : [];
    asksError = result.error;
  } else {
  ...
  const openAsks = askParam ? sortedAsks : sortedAsks.filter((a) => a.status === "open");
  const closedAsks = askParam ? [] : sortedAsks.filter((a) => a.status !== "open");

  function renderAskCard(ask: any) {
    const isMine = ask.requester_id === userId;
    return (
  ...
  return (
    <div>
      ...
      {askParam && sortedAsks[0]?.content_flag && (
        <p className="mb-2 font-mono text-xs text-commons-brick">
          Viewing flagged content — normally hidden from Asks.
        </p>
      )}

      {askParam ? (
        <a href="/asks" className="mb-4 inline-block font-mono text-xs font-bold underline">
          ← back to Asks
        </a>
      ) : (
      ...
      <div className="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3">
        ...
        {openAsks.length === 0 && closedAsks.length === 0 && (
          <p className="font-mono text-sm">
            {askParam ? "That ask couldn't be found." : "No asks yet — be the first!"}
          </p>
        )}
      </div>

      {!askParam && openAsks.length === 0 && closedAsks.length > 0 && (
        <p className="font-mono text-sm">No open asks right now.</p>
      )}

      {!askParam && closedAsks.length > 0 && (
